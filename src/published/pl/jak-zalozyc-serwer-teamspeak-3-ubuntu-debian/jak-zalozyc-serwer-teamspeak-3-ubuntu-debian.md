...
locale: pl
category: "Poradniki"
tags: ["teamspeak", "komunikator", "ubuntu", "debian"]
status: published
author:
  name: VoxiHost Team
...
contributors:
  ...
  - danielmarszalkowski
faq:
  - question: "Jak połączyć się z serwerem TeamSpeak 3 po raz pierwszy?"
    answer: "Aby połączyć się po raz pierwszy, wpisz adres IP swojego serwera VPS w kliencie TeamSpeak 3. Po połączeniu serwer zapyta o klucz administratora (Privilege Key). Wklej klucz wygenerowany w Kroku 5, aby otrzymać pełne uprawnienia Server Admin."
  - question: "Jakie porty należy otworzyć w zaporze sieciowej dla TeamSpeak 3?"
    answer: "Musisz otworzyć porty: <code>9987/udp</code> (ruch głosowy), <code>10011/tcp</code> (ServerQuery) oraz <code>30033/tcp</code> (transfer plików). Jeśli używasz UFW, dodaj odpowiednie reguły zezwalające na te połączenia."
  - question: "Jak odzyskać utracony klucz Privilege Key?"
    answer: "Nowy klucz Privilege Key można wygenerować za pomocą ServerQuery lub bezpośrednio w kliencie TS3 (jeśli masz już innego admina). Możesz również sprawdzić logi serwera w katalogu <code>logs/</code> lub wygenerować nowy za pomocą narzędzi query."
  - question: "Czy serwer TeamSpeak 3 na VPS jest darmowy?"
    answer: "Pobranie i uruchomienie serwera TeamSpeak 3 jest darmowe dla użytku niekomercyjnego do 32 slotów (bez dodatkowej licencji). Jeśli potrzebujesz więcej slotów lub planujesz komercyjne wykorzystanie, konieczne jest zakupienie licencji od TeamSpeak."
  - question: "Jak zaktualizować serwer TeamSpeak 3 do najnowszej wersji?"
    answer: "Aby zaktualizować serwer, zatrzymaj usługę poleceniem <code>sudo systemctl stop teamspeak</code>, pobierz najnowsze pliki serwera z oficjalnej strony i nadpisz nimi stare pliki (uważając, aby nie nadpisać bazy danych <code>ts3server.sqlitedb</code> oraz plików konfiguracyjnych). Następnie ponownie uruchom usługę."
howto:
